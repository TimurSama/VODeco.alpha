...
### 4.2. ⚠️ КРИТИЧНО: Добавьте DATABASE_URL ПЕРЕД первым деплоем!

**DATABASE_URL должен быть добавлен ДО первого деплоя, иначе сборка упадёт!**
...
2. **Проверьте, что DATABASE_URL добавлен:**
   - Settings → Environment Variables
   - Должна быть переменная `DATABASE_URL`
   - Значение должно начинаться с `postgresql://`

**Если DATABASE_URL не добавлен:**
- Сборка упадёт с ошибкой "Environment variable not found: DATABASE_URL"
- Создайте базу данных и повторите деплой
...
### 5.1. ⚠️ ВАЖНО: Убедитесь, что DATABASE_URL добавлен!

**Перед деплоем обязательно:**
1. Проверьте, что `DATABASE_URL` добавлен в Environment Variables
2. Если нет - создайте PostgreSQL базу данных (см. Шаг 4.2)
...
**Если есть ошибка "Environment variable not found: DATABASE_URL":**
- ⚠️ **Создайте PostgreSQL базу данных** (Storage → Create Database → Postgres)
- ⚠️ **Проверьте**, что `DATABASE_URL` добавлен в Environment Variables
- ⚠️ **Повторите деплой**
...
### ⚠️ ВАЖНО: Миграции выполняются автоматически или вручную

**Вариант 1: Автоматически (рекомендуется)**

Миграции будут применены автоматически при первом запросе к API, если база данных пуста.
...
### ⚠️ Если миграции не применены автоматически:

Можно использовать `prisma db push` вместо миграций для MVP:

```bash
npx prisma db push
```

Это создаст таблицы напрямую без миграций (подходит для MVP).
